...
public class falscheStrategie {
    ...
    public static void createTable(){
        ...
        try {
            ...
            System.out.println("* Tabelle FalscheStrategie erstellen, falls nicht vorhanden");
            String sql = "CREATE TABLE if not exists FalscheStrategie(Datum date, Aktie varchar(10), Kaufen boolean, Anzahl Integer, Depot double, PRIMARY KEY(Datum));";
    ...
    public static void dummie(LocalDate ld){
        try {
            ...
            String sql = "INSERT IGNORE INTO FalscheStrategie values('"+ld+"','"+aktie+"',"+0+","+anzahlaktien+","+depot+");";
    ...
    public static void dbEintrag(int oansOdaNull){
        try {
            ...
            String sql = "INSERT IGNORE INTO FalscheStrategie values('"+temp+"','"+aktie+"',"+oansOdaNull+","+anzahlaktien+","+depot+");";
            //System.out.println(sql);
            myStat.execute(sql);
    ...
    public static void getLetztenEintrag(){
        ...
        try {
            conn = DriverManager.getConnection("jdbc:mysql://"+hostname+"/"+dbname+"?user="+user+"&password="+password+"&serverTimezone=UTC");
            Statement myStat = conn.createStatement();
            String sql = "Select * from FalscheStrategie where aktie='"+aktie+"' order by Datum desc limit 1;";
            ResultSet reSe=myStat.executeQuery(sql);
    ...
    public static void loeschDatum(LocalDate ld){
        try {
            ...
            Statement myStat = conn.createStatement();
            String sql = "Delete from FalscheStrategie where Datum='"+ld+"' and aktie='"+aktie+"';";
            //System.out.println(sql);
            myStat.execute(sql);
